// Diccionario de idioma
var transale = {
	'menu' : {
		'm-ph1':{
			'en' : 'PORTAFOLIO',
			'es' : 'PORTAFOLIO'
		},
		'm-ph2':{
			'en' : 'ABOUT US',
			'es' : 'NOSOTROS'
		},
		'm-ph3':{
			'en' : 'WORK',
			'es' : 'TRABAJO'
		},
		'm-ph4':{
			'en' : 'CONTACT US',
			'es' : 'CONTACTO'
		}
	},

	'contact':{
		'cph1':{
			'en' : "We'd love to hear from you.",
			'es' : 'Queremos saber de ti.'
		},
		'cph2':{
			'en' : 'Studios.',
			'es' : 'Estudios.'
		},
		'cph3':{
			'en' : 'Italy',
			'es' : 'Italia'
		},
		'cph4':{
			'en' : '<span class="black">Alexander Tadlock - </span> General Manager',
			'es' : '<span class="black">Alexander Tadlock - </span> Gerente General'
		},
		'cph5':{
			'en' : 'Mexico',
			'es' : 'México'
		},
		'cph6':{
			'en' : '<span class="black">Enrique Hinzpeter - </span> General Manager',
			'es' : '<span class="black">Enrique Hinzpeter - </span> Gerente General'
		},
		'cph7':{
			'en' : 'Sepa la chucha # 139, Mexico City. Mx.',
			'es' : 'Sepa la chucha # 139, Ciudad de México. Mx.'
		},
		'cph8':{
			'en' : 'About your project.',
			'es' : 'Sobre su proyecto.'
		},
		'cph9':{
			'en' : "For general enquiries or anything you'd like to discuss, please contact us here.",
			'es' : 'Para consultas generales o sobre lo que desees hablar, por favor contáctanos aquí.'
		},
		'cph10':{
			'en' : 'Name.',
			'es' : 'Nombre'
		},
		'cph11':{
			'en' : 'E-mail.',
			'es' : 'Correo electrónico.'
		},
		'cph12':{
			'en' : 'Message.',
			'es' : 'Mensaje.'
		},
		'cph13':{
			'en' : 'Send',
			'es' : 'Enviar'
		}
	},

	'footer':{
		'f-ph1':{
			'en' : 'Empowering visual capital.',
			'es' : 'Potenciación del capital visual.'
		},
		'f-ph2':{
			'en' : 'BRANDING',
			'es' : 'MARCA'
		},
		'f-ph3':{
			'en' : 'INFORMATION DESIGN',
			'es' : 'INFORMACIÓN DE DISEÑO'
		},
		'f-ph4':{
			'en' : 'MARKETING',
			'es' : 'MARKETING'
		},
		'f-ph5':{
			'en' : '© 2016 Le Brand capital. All Rights Reserved.',
			'es' : '© 2016 Le Brand capital. Todos los derechos reservados.'
		}
	} ,
	'project':{

		'lets':{
			'en':'Let\'s start your project.',
			'es':'Empecemos tu proyecto.',
		},
		'agora':{
		'en' : 'Identity + Branding + Web Design for the new plaza boutique located at Tuxtla Gutierrez, Chiapas.\
		          <br><br>\
		        <strong> Le Brand  </strong> collaborated with the firm of architects responsible for developing the new plaza boutique at Chiapas, in order to better understand the architectonic concept for the project. This collaboration helped to cultivate the conceptual aspects of the branding   <strong> Le Brand  </strong> developed for the project. The process started with marketing research, which helped to achieve the best functional naming of the plaza, as well as the complete development of the branding, web design, and specific applications of the project.',
		'es' : 'Identidad + Branding + Diseño Web para una nueva plaza boutique en la ciudad de Tuxtla Gutiérrez, Chiapas.\
		          <br><br>\
		        <strong> Le Brand  </strong> colaboró junto con una firma de arquitectos para comprender el concepto arquitectónico del proyecto y crear de manera congruente la identidad e  imagen de la nueva plaza boutique en Chiapas.  El proceso inició desde la investigación de mercados para la selección adecuada del naming del proyecto, así como el desarrollo del branding, diseño web, elementos arquitectónicos y aplicaciones específicas.'
				},
	'agroamericana': {
		'en' : 'Identity + Branding + Web Design for the agribusiness holding with offices in Mexico, California and New York. \
			 <br><br>\
        			Agroamericana works within the 4 sectors of the Agro-business: the farming, harvest, distribution, and marketing of agricultural products. <strong>Le Brand</strong> had the challenge of creating a unique identity for this international holding. <strong>Le Brand</strong> created the branding, web design, and communication strategies needed in order to help introduce the Agroamericana brand into North American markets and connect them with Latin American agribusiness companies.',

		'es' : 'Identidad + Branding + Diseño Web para el holding de agronegocios con oficinas en México, California y Nueva York .\
			 <br><br>\
		 	Agroamericana interviene en los cuatro segmentos del agro: El cultivo, la cosecha, la distribución y la comercialización con estrategias y planes ejecutivos de productos perecederos. Le Brand tuvo el reto de crear la imagen de esta empresa internacional, la cual albergaría dentro de ella más empresas con funciones específicas dentro del mundo de los agronegocios.\
		 	 <br><br>\
		  	Se hizo la creación de la marcas así como el desarrollo del diseño web y diversas redes sociales para poder incursionar la marca Agroamericana en el mercados de Estados Unidos. '
	},

	'ah_muzen' : {

		'en' : 'Identity + Branding for a Honey Bee’s company in the state of Quintana Roo, Mexico. In 2014 the Honey Bee company Ah Muzen Kab expressed a desire to have a solid and structured identity and brand for the company’s objectives.\
		                <br><br>\
		         Ah Muzen Kab reached out to <strong> Le Brand </strong> for the next challenge in the process of their development: configuring the identity and branding for the unique and organic honey bee of the Mayan jungles from Quintana Roo that would be commercialized and exported to different countries all around the world such as Japan, Germany, and the United States. <strong> Le Brand </strong> used marketing research to create a unique identity for the company and to develop branding that met the quality required to be competitive in international markets, without compromising the regional aspects of the project and the company. <strong> Le Brand </strong> helped Ah Muzen Kab foster and fortify an identity that reflects the culture of the Mayan people and their traditional beekeeping processes through a carefully designed illustration and well-selected typography.\
		            <br><br>\
		          The new image was quickly adopted by the project and is currently being used as the brand identity for the products, packages, and social networks of Ah Muzen Kab.\
		    	</p>',

		'es' : 'Identidad + Branding + Diseño Web para la sociedad cooperativa apicultora “Ah Muzen Kab” del Estado de Quintana Roo.\
		                <br><br>\
		         La sociedad cooperativa apicultora de Quintana Roo, Ah Muzen Kab, iniciativa encargada de la recolección, producción y comercialización de miel orgánica de la selva maya. Con el objetivo de formalizar una identidad e imagen que pudiese posicionar a la empresa acuerdieron a <strong> Le Brand </strong> para potencializar el capital visual del proyecto aspirando a ser altamente competitivo en países alrededor del mundo como: Japón, Alemania, Estados Unidos.\
		            <br><br>\
		          Se creó un lineamiento de diseño que tuviera el nivel y las pautas para poder competir en los mercados internacionales sin que perdiera  la escencia de los procesos tradicionales y orgánicos de la apicultura regional mexicana. Es así como se desarrolló un branding que cumpliera con todas las características mencionadas, enfatizadas en la resolución del cuidado y detalle de la ilustración de la marca Ah Muzen Kab.\
		          <br><br>\
		         La nueva imagen fue adoptada con gusto por la sociedad cooperativa y comenzó a ser empleada para sus productos, empaques y redes sociales.\
		    	</p>'
	},

	'aimalaion' : {

		'en' : 'Identity + Branding + Web design for a studio and record producer in Cholula, Puebla.\
		            <br><br>\
		        Aimalaion is a Mexican recording studio established in 2013 by a musician and audio engineer from the city of Puebla who has many years of experience in themusic recording industry. In order to develop Aimalaion, he decided to create a new branding design to reflect the personality of his vision as well as his passion for the projects and collaborations the studio would offer.  <strong> Le Brand </strong>  designed the studio’s identity and branding, emphasizing the professionalism, high quality, andinnovation of the new studio as well as the artist’s personal interests, such as tattoos and new sounds.\
		          <br><br>\
		        The logo that  <strong> Le Brand </strong>  has created for Aimalaion centers on an illustration of a lion. The design includes a fluorescent spectrum of colors that received very positive feedback from the team at Aimalaion as well as some local musicians.',

		'es' : 'Identidad + Branding + Diseño Web para un estudio/ casa productora musical en Cholula, Puebla.\
		            <br><br>\
		        Aimalaion es un estudio / productora musical emprendida por un grupo de músicos e ingenieros en audio poblanos, con más de 10 años de experiencia en la industria de la música. El proyecto carecia de identidad visual por lo que la firma <strong> Le Brand </strong>  fue contactada para la elaboración de la identidad corporativa.\
		          <br><br>\
		        Como parte central de la imagen se creó el mensaje figurativo de un león junto con una colorida gama de pantones fluorescentes que tuvo buen recibimiento por parte del equipo del estudio y algunos de los artistas locales que asisten a las grabaciones.'
	},

	'amerivacs' : {
		'en' : 'Identity + Branding + Web Design for a national American vacuum sealers company.\
        		<br> <strong>AmeriVacS</strong> is an established national American vacuum sealers company that builds vacuum sealers that exceed industry standards for design integrity, component quality, speed, redundancy, and failsafe mechanisms. These qualities are evident in every sealer they produce: from the very first, to their latest medical vacuum sealer.\
        		<br>\
        		Amerivacs came to <strong>Le Brand</strong> in need of a fresh identity, information structuring, and branding strategies for new target markets. <strong>Le Brand</strong> studied the company in order to introduce new freshness to the company and reinforce the high quality values that Amerivacs has established for 20 years.',
		'es' : 'Identidad + Branding + Diseño Web para la identidad de la empresa Americana especializada en elaboración de maquinas de sellado al vacio, con gran trayectoria nacional. <br>Amerivacs es una empresa americana con presencia nacional e internacional con más de 20 años de experiencia en el mercado. Dentro de los planes estratégicos de la empresa, Amerivacs requeria actualizar su identidad para acercarse a nuevos clientes potenciales. Le Brand tuvo la ardua tarea de analizar y conceptualizar gráficamente, los atributos y altos estándares de calidad con los que esta empresa americana opera a nivel nacional.<br> Actualmente, los directivos de Amerivacs quedaron totalmente satisfechos con la nueva identidad de la empresa, ya que refleja en lo absoluto, ya prestigiada trayectoria del proyecto.'
	},

	'centennial' : {

		'en' : 'Identity + Branding + Web Design for a Californian new agribusiness company.\
		        <br><br>\
		        Centennial Organics is a company from Agrobusiness Holding of Agroamericana.\
			This company specializes in the sale, distribution, and marketing of organic Californian produce. The challenge of this project was to create an identity for the\
			company that could compete with the already established brands on the market such as Dole, Chiquita, and Driscoll’s in Whole Food Markets. Vigorous marketing research and assessment of consumer’s behavior approaches helped <strong> Le Brand</strong>  determine the most accurate naming for the project. This research also lead to the development of a very well detailed illustration concept that strongly imparts the meaning of a traditional American family farms company',

		'es' : 'Identidad + Branding + Diseño Web para una comercializadora californiana de frutas y verduras que distribuye sus productos en los principales supermercados del estado de California, Estados Unidos.\
		        <br><br>\
		        Centennial Organics es parte del holding de agronegocios internacional de Agroamericana. Esta empresa se dedica a la venta, distribución y mercadeo de productos orgánicos mexicanos más alla de las fronteras nacionales. El reto fue crear una Identidad que reflejara los altos estándares de la empresa y que pudiera hacerle frente a  marcas americanas nacionales en diversos puntos de venta como Whole Food Markets y otras tiendas de gran prestigio.'
	},

	'cmx' : {

		'en' : 'Identity + Branding + Web Design for the project Conectando México foundation, with the objective of giving free Internet educational purpose access for specific marginal communities in Mexico city.\
		            <br><br>\
		        The Conectando Mexico foundation’s objective is to raise money in order to grant free internet access to marginalized and targeted areas of Mexico City. They hope to give thousands of people the chance to access the vast information, education, and training resources the Internet has to offer. The Conectando Mexico, with the help of other educational organizations, hopes to use the Internet to enrich the education of thousands of people.  <strong>Le Brand </strong> had the incredible opportunity to help with this project by creating the branding, web design, and promotional applications needed by the CMX project.',

		'es' : 'Identidad + Branding + Diseño Web para la fundación Conectando México, con el propósito de proveer de Internet gratuito con fines educativos en algunas de las zonas más marginadas de la Ciudad de México.\
		            <br><br>\
		        La fundación Conectando México está integrada por diferentes ciudadanos y empresarios comprometidos con la educación de calidad en México a través de herramientas digitales como el Internet, llevando a las zonas más marginadas de la Ciudad de México, una oportunidad de superación educativa a las comunidades.\
		            <br><br>\
		              <strong> Le Brand </strong>  tuvo la gran oportunidad de poder contribuir a este proyecto creando la  identidad y el desarrollo complete del branding,así como diversas aplicaciones requeridas.\
		            <br><br>\
		 	El branding creado para esta fundación fue muy bien tomado por los patrocinadores; Actualmente se sigue usando para promover y expandir estafundación en todo el país.'
	},

	'elsa' : {

		'en' : 'Identity + Branding + Web Design for the well-known architect Elsa Elena, who has a bright future in the architecture and interior design fields. <br>Elsa Elena contacted <strong>Le Brand</strong> with the objective of redesigning her personal branding and website design. After coming to understand the essence of her work through strategic interviews, <strong>Le Brand</strong> created a strong and coherent design that showcased Elsa Elena’s identity and architectonic style. <br><strong>Le Brand’</strong>s proposal was very well received by the architect and by her clients. Elsa Elena expressed that the branding represented her work and herself excellently.',

		'es' : 'Identidad + Branding + Diseño Web para la renombrada arquitecta y especialista en interiorismo, Arq. Elsa Elena González Labrada, la cual lleva una gran trayectoria en la decoración y la arquitectura de interiores al participar en diferentes obras y proyectos en la Ciudad de México, Acapulco, Puerto Arista y Chiapas.\
		    		 Elsa Elena se acercó a Le Brand con la petición de configurar el diseño de su branding personal. Tras concluir con una exhaustiva investigación de su trabajo, analizar los elementos que la caracterizan, los estilos y colores con los que mástrabaja; Solucionamos la problemática creando una imagen que reflejase tanto el estilo único de la arquitecta como la trayectoria y el legado de sus obras. \
		    		 Nuestra propuesta de branding fue muy bien aceptada por la arquitecta, así como por sus clientes ya que representaba fielmente su trabajo y personalidad.'
	},

	'town':{
		'en' : 'Identity + Branding + Web Design for the Fighter Town Brewing Co. In 2016 will be San Diego’s primer contract brewer.\
		        <br>\
		        Fighter Town is a local Californian contract craft brewery that assists craft breweries in reaching their potential through strategic partnership and lucrative tasting rooms. Fighter Town approach <strong>Le Brand</strong> with the challenge of creating an identity that could stand out and act as an example for other local craft breweries competing against the established national brands that control more than the 90% of the national market.\
		        <br>\
		        <strong>Le Brand</strong> responded to the call of duty for this important mission. The victory of the local allies against the commercial monster brands depended on the creation of a very powerful weapon: The Fighter Town company’s branding.\
		        <br> Through many strategic reunions with the commanders of this mission, extensive marketing research, and some true Californian inspiration such as the Top Gun movie, <strong>Le Brand</strong> lead this amazing contract craft beer project to victory. <strong>Le Brand</strong> delivered the identity and image required for the battlefield.\
		        <br> \
		        The illustration and the original typography design created by <strong>Le Brand</strong> were accepted with high praise by the investors and the users. Right now Fighter Town is under construction and will soon be ready to start expanding from California to therest of the United States and the world.',

		'es' : 'Identidad + Branding + Diseño Web para la empresa Fighter Town Brewing Co. La cual será en 2016 la primera cervecería/ embotelladora artesanal en San Diego, California.\
		        <br> <br>\
		        Fighter Town quiere ayudar a cervecerías artesanales de toda la región alcanzar su máximo potencial a través de alianzas estratégicas, grandes producciones a menores precios y salas de degustación. Fighter Town se acercó a Le Brand con el reto de crear una marca e imagen que fuese visualmente imponente para poder competir contra las grandes marcas comerciales de cerveza que acaparan más del 90% del mercado nacional.\
		        <br> <br>\
		        La ilustración, así como el diseño original de la tipografía fue muy bien aceptada por los inversionistas y los usuarios. Actualmente Fighter Town esta en construcción y listo para expandirse abrir sus puertas a fin del año.'
	},

	'capital' : {
		'en' : 'Identity + Branding + Web design for the biggest marketing of electric automotive parts of the Mexican southwest region.\
		         <br>\
		         <br>\
		         Grupo Chapital came to  <strong> Le Brand </strong> with the desire to strengthen the image of their already well-known 4th generation family’s company name. Grupo Chapital wanted to establish a unique identity for their company and establish a presence in the digital world. As with many other businesses in Mexico, there have been few resources invested in the corporate image, marketing, and communication aspects of this company. Grupo Chapital reached out to <strong> Le Brand </strong>  in search of new branding that could represent all of the company’s essential motives and desires, in order to foster expansion and allow the company to stand out in competitive regional markets.\
		          <br>\
		          <br>\
		          <strong> Le Brand </strong>  also had the opportunity to create an array of diverse applications of the new branding that will be in direct contact with the customer.  This branding will reinforce the motives and desires of Grupo Chapital. The new branding has been very well received by users, due to the fact that it is visually attractive andaccurately describes Grupo Chapital’s experience and expertise.',

		'es' : 'Identidad + Branding + Diseño Web para la mayor comercializadora de autopartes y refacciones automotrices del Sureste de México.\
		         <br>\
		         <br>\
		         Grupo Chapital requería de nuevas estrategias en su imagen corporativa, deseaba expandir el número de usuarios e incursionar en el mundo digital. \
			Como cualquier otro negocio en México, había enfocado pocos recursos e interés en la creación de su imagen y en los usuarios potenciales por medio del Internet. Grupo Chapital se acerco a Le Brand para diseñar la marca que representara  mejor todos sus atributos y de esta forma poder unificar su posiciónamiento empresarial en la ciudad capital de Tuxtla Gutiérrez en el estado de Chiapas. Además, hubo la oportunidad de crear varias aplicaciones relacionadas directamente con los usuarios y de esta forma generar lazos de identidad y experiencia de marca con los consumidores.\
		          <br>\
		          <br>\
		          La nueva imagen fue muy bien percibida por los usuarios ya que era visualmente atractiva y representaba las características de dicha empresa, mientras que el desarrollo del sitio web, fue un gran paso en la  busqueda de nuevo usuarios a través medio digital preferente para el posicionamiento de la marca en la mente del público actual.'
	},

	'jltc' : {

		'en' : 'Identity + Branding + Web Design for JLTC Holding, with a variety of financial companies but it mainly specialize in Agro-business, Financial investment and real state. \
		<br><br>\
        			JLTC holding is a financial company that possesses a large amount of capital and many years of experience. JLTC enlisted the help of Le Brand \
      			  in creating a new image in order to communicate the company’s strengths and professionalism to their future investors and financial associates. \
       			 Le Brand did this by creating a very detailed branding that completely went above and beyond the client’s expectations. \
       			 <br><br>\
        			The new image was a great success with the businessman and new investors.\
       			 JLTC was so happy with the results that they asked Le Brand to continue working on their website and their social media networks. ',

		'es' : 'Identidad + Branding + Diseño Web para el Holding familiar JLTC, el cual entre sus muchos campos se especializa en los agronegocios, inversión financiera y bienes raíces. \
			<br><br>\
			JLTC es una empresa financiera con gran capital y muchos años de experiencia, la cual requería de una nueva imagen para poder transmitir esta fortaleza y seguridad de hacer negocios con dicho Holding financiero a sus futuros inversionistas y socios. Le Brand decidió solucionar esto al crear un sello auténtico como imagotipo para reflejar los valores que engloban a la empresa. \
			<br><br>\
			La nueva imagen tuvo gran aceptación entre los empresarios y nuevos inversionistas, debido a esto se continuó el proyecto generando más estrategias de comunicación y marketing para el proyecto. '
	},

	'marmoline' : {

		'en' : 'Identity + Branding + Web Design for one of the most important marble companies inn the Mexico’s southeastern region. The company extracts, distributes and sells out marble pieces for architectonic purposes.\
		              <br><br>\
		        Marmoline contacted <strong> Le Brand </strong> in search of an urgent re-design in order to consolidate their identity and make their company a more attractive option for customers.\
		              <br><br>\
		          After conducting marketing research on all the different process that the company was involved with, <strong> Le Brand </strong> created new branding that reflected the characteristics that the company wished to communicate to the users, such as elegance, quality, excellent design, and sobriety.',

		'es' : 'Identidad + Branding + Diseño Web para una de las empresas marmoleras más importantes del sureste de México. La empresa Marmoline se encarga de la extracción, venta y distribución de una gran variedad de piezas de mármol utilizados con fines de construcción.\
		              <br><br>\
		       Marmoline se acercó a Le Brand para poder diseñar su identidad y así consolidar una empresa más fuerte y atractiva para sus clientes.\
		              <br><br>\
		         Después de una larga investigación sobre todos los procesos operativos de la empresa, así como la calidad de sus materiales y acabados de los productos, se desarrolló la nueva identidad de Marmoline,  la cual refleja los valores centrales del proyecto.'
	},

	'slice' : {

		'en' : 'Identity + Branding + Web Design for a Mexican-Californian fresh produce agribusiness brand.\
		        <br>\
		        Sunslice’s consumer’s desires were critical in determining the process of creating a brand for the company. While developing branding for Sunslice, <strong>Le Brand</strong> was asked to create very high quality visuals that would meet the high expectations of millennial consumers. Sunslice was looking for branding that would seem friendly and sustainable, and would represent the company with a healthy personality. At the same time, Sunslice wanted the designs to be forceful, sober, and minimalistic. <strong>Le Brand</strong> met these demands with enthusiasm and Sunslice was not disappointed.',

		'es' : 'Identidad + Branding + Diseño Web para la empresa mexicana-americana comercializadora de productos con la mejor calidad. Protagonistas de la industria agrícola en México al ser grandes exportadores y mantener un negocio sustentable.\
		        <br> <br>\
		       Crear el branding de Sunslice fue todo un reto ya que tenía que competir con marcas norteamericanas dentro del mercado nacional de Estados Unidos en tiendas de auto- servicios y Whole Foods market. Debía contar con una personalidad sustentable, amigable con el medio ambiente y saludable mientras que al mismo tiempo debía de representar la vanguardia minimalista, la seriedad y el prestigio de la empresa. La marca de Sunslice fue un éxito ya que tuvo gran aceptación en el mercado norteamericano gracias a las estrategias de comunicación implementadas en las plataformas digitales.'
	},

	'tulum' : {

		'en' : 'Identity + Branding + Web Design for the local government administration of Tulum image of 2014- 2016.\
		                <br><br>\
		         Candidates in the mayoral election of Tulum invited <strong> Le Brand </strong> to create a proposal for the new image of the government of Tulum. <strong> Le Brand </strong> conducted research on the history, culture, geography, plants, and animals of Tulum in order to decide what elements represent Tulum the best. A proposal was created for the new web page of the Town hall and designs were submitted for the many applications and ads Tulum uses to make citizens aware of social and government programs.',

		'es' : 'Identidad + Branding + Diseño Web para la identidad de la administración del Gobierno de Tulúm de 2014 a 2016.\
		                <br><br>\
		        El electo candidato de Tulúm invitó a Le Brand a participar en la creación de una propuesta para la nueva imagen del gobierno municipal de Tulúm en la administración que tomaría poder en 2014. Se hizo una gran investigación sobre la historia, cultura, geografía, fauna y flora de Tulúm para poder delimitar que elementos caracterizarían mejor la imagen gubernamental.  También se hizo una propuesta de la nueva página web del ayuntamiento así como diferentes aplicaciones y estrategias comunicativas para dar a conocer a la ciudadanía, los distintos programas que ofrecería este nuevo gobierno.'
	},

	'vintage' : {
		'en' : 'Editorial engineering, marketing strategies, naming and complete branding for Vintage magazine, a magazine focusing on topics such as music, design, and art from Puebla, Mexico.\
		              <br><br>\
		         Vintage magazine is the point of origination of <strong> Le Brand </strong> . After years of studying together, Partners Alexander Tadlock and Enrique Hinzpeter began their first design project back in 2013. This project consisted of creating an entire commercialmagazine as a final project for their university. Not only was <strong> Le Brand </strong> founded by this partnership, but also the very first professional project of <strong> Le Brand </strong> was created with such uniqueness that it was awarded the Best National University Editorial Project 2013 by the Mexican Design Association.  This project was also recognized by the Worldwide Editorial Gustavo Gili and the Mexican cultural Fondo de Cultura Economica, and received an honored mention by the Universidad de las Americas Puebla (UDLAP).',

		'es' : 'Ingeniería Editorial en la revista Vintage la cual fue premiada como Mejor Proyecto Editorial Universitario Nacional 2013 por la asosiación mexicana de diseño, la Editorial Gustavo Gili y el Fondo de Cultura Económica.\
		              <br><br>\
		         Este fue el primer proyecto de Le Brand el cual consistió en la creación de la Ingeniería Editorial así como todos los contenido de la revista Vintage. Este proyecto participó en el certamen del Fondo de Cultura Económica de Proyectos Editoriales Universitarios compitiendo con Universidades de toda la República. La revista Vintage el primer lugar de acuerdo a la competencia. Fue publicada en varias páginas web y reconocida por la Universidad de las Américas Puebla (UDLAP).'
	}

	},
	'index':{
		'ph1':{
			'en' : "New plaza boutique located at Tuxtla Gutierrez, Chiapas.",
			'es' : ' Nueva  plaza boutique en la ciudad de Tuxtla Gutiérrez, Chiapas.'
		},
		'ph2':{
			'en' : 'Honey Bee’s company in the state of Quintana Roo, Mexico.',
			'es' : 'Sociedad cooperativa apicultora DzAh Muzen Kabdz del Estado de Quintana Roo.'
		},
		'ph3':{
			'en' : 'Studio and record producer in Cholula, Puebla.',
			'es' : 'Casa productora musical en Cholula, Puebla.'
		},
		'ph4':{
			'en' : 'The biggest marketing of electric automotive parts of the Mexican southwest region.',
			'es' : '<strong> Comercializadora </strong> de autopartes y refacciones automotrices del Sureste de México.'
		},
		'ph5':{
			'en' : 'Californian new agribusiness company.',
			'es' : 'Comercializadora californiana de frutas y verduras en Estados Unidos.'
		},
		'ph6':{
			'en' : 'Conectando México foundation, has the objective of giving free Internet with educational purpose.',
			'es' : 'Fundación Conectando México tiene el propósito de proveer de Internet gratuito con fines educativos.'
		},
		'ph7':{
			'en' : 'Fighter Town Brewing Co. In 2016 will be San Diego’s primer contract brewer.',
			'es' : 'Será en 2016 la primera cervecería/ embotelladora artesanal en San Diego, California. '
		},
		'ph8':{
			'en' : 'One of the most important marble companies inn the Mexico’s southeastern region.',
			'es' : 'Empresa marmolera más importantes del sureste de México.'
		},
		'ph9':{
			'en' : 'JLTC Holding, with a variety of financial companies but it mainly specialize in Agro-business, Financial investment and real state.',
			'es' : 'Holding familiar JLTC, el cual se especializa en los agronegocios, inversión financiera y bienes raíces.'
		},
		'ph10':{
			'en' : 'Agrobusiness holding with offices in Mexico, California and New York.',
			'es' : 'Holding de agronegocios con oficinas en México, California y Nueva York .'
		},
		'ph11':{
			'en' : 'The well-known architect Elsa Elena, who has a bright future in the architecture and interior design fields.',
			'es' : 'Renombrada arquitecta y especialista en interiorismo, Arq. Elsa Elena González Labrada.'
		},
		'ph12':{
			'en' : 'Mexican-Californian fresh produce agribusiness brand.',
			'es' : 'Empresa mexicana-americana comercializadora de productos con la mejor calidad.'
		},
		'ph13':{
			'en' : 'The local government administration of Tulum image of 2014- 2016.',
			'es' : 'Identidad de la administración del Gobierno de Tulúm de 2014 a 2016.'
		},
		'ph14':{
			'en' : 'A magazine focusing on topics such as music, design, and art from Puebla, Mexico.',
			'es' : 'Revista Vintage la cual fue premiada como Mejor Proyecto Editorial Universitario Nacional 2013 por la asosiación mexicana de diseño.'
		},
		'ph15':{
			'en' : 'A national American vacuum sealers company.',
			'es' : 'Empresa Americana especializada en elaboración de maquinas de sellado al vacio, con gran trayectoria nacional.'
		},
		'ph16':{
			'en' : 'A big smile in every bite! Sunslice foods offers the best fresh produce in California.',
			'es' : 'Una gran sonrisa en cada bocado! Alimentos Sunslice ofrece los mejores productos frescos en California.'
		},
		'ph17':{
			'en' : 'The victory of the local allies against the commercial monster brands depended on the creation of a very powerful weapon: The Fighter Town company’s branding.',
			'es' : 'La victoria de los aliados locales contra las marcas comerciales monstruo dependía de la creación de un arma muy poderosa : la marca de la compañía Fighter Town.'
		},
		'ph18':{
			'en' : 'Amerivacs came to Le Brand in need of a fresh identity, information structuring, and branding strategies for new target markets.',
			'es' : 'Amerivacs vino a Le Brand en la necesidad de una identidad fresca, estructuración de la información, y estrategias de marca para los nuevos mercados de destino.'
		},
		'ph19':{
			'en' : 'Agroamericana works within the 4 sectors of the Agro-business: the farming, harvest, distribution, and marketing of agricultural products. Le Brand had the challenge of creating a unique identity for this international holding.',
			'es' : 'Agroamericana interviene en los cuatro segmentos del agro: El cultivo, la cosecha, la distribución y la comercialización con estrategias y planes ejecutivos de productos perecederos. Le Brand tuvo el reto de crear la imagen de esta empresa internacional.'
		}

	},

	'aboutUs':{
		'au1':{
			'en': 'Hello.',
			'es': 'Hola.'
		},
		'au2':{
			'en': 'This is our Manifest',
			'es': 'Este es nuestro manifiesto'
		},
		'au3':{
			'en':'is an identity and branding expertise design firm run by the global partners Alexander Tadlock and Enrique Hinzpeter since 2013.',
			'es':'es una firma especializada en la creación de identidad y Branding a cargo de los socios globales Alexander Tadlock y Enrique Hinzpeter desde el año 2013.'
		},
		'au4':{
			'en':'We are always one step ahead due to our globalized operation system and our world-wide visual culture. We believe in the power of visual capital that every individual project represents.',
			'es':'Siempre estamos un paso por delante gracias a nuestro sistema de operación global y nuestra cultura visual mundial. Creemos en el poder del capital visual en cada uno de nuestros proyectos.'
		},
		'au5':{
			'en':'This is our way.',
			'es':'Esta es la forma en la que trabajamos.'
		},
		'au6':{
			'en':'Welcome.',
			'es':'Bienvenido.'
		},
		'au7':{
			'en':'Brand system',
			'es':'Sistema de Branding'
		},
		'au8':{
			'en':'Build inside to create outside.',
			'es':'CONSTRUYE ADENTRO  PARA  CREAR AFUERA'
		},
		'au9':{
			'en':'Always that a Brand is designed, it is essential to <span class="bold">build the DNA of the Brand Identity</span>, Conceptual idea, Personality, Values and Goals, Behavior and Relationships.',
			'es':'Siempre que se diseñe una marca, es esencial  <span class="bold">construir el ADN de la identidad de marca</span> : Idea conceptual, Personalidad,Valores y Metas, Comportamiento y Relaciones'
		},
		'au10':{
			'en':'DNA',
			'es':'ADN'
		},
		'au11':{
			'en':'Conceptual Idea',
			'es':'Idea Conceptual'
		},
		'au12':{
			'en':'Behavior',
			'es':'Comportamiento'
		},
		'au13':{
			'en':'Personality',
			'es':'Personalidad'
		},
		'au14':{
			'en':'Relationship',
			'es':'Relaciones'
		},
		'au15':{
			'en':'Values and Goals',
			'es':'Valores y Metas'
		},
		'au16':{
			'en':'Identity',
			'es':'Identidad'
		},
		'au17':{
			'en':'<span class="diferent">INSTEAD OF TRYING TO BE DIFFERENT</span><br> BE MEANINGFUL',
			'es':'<span class="diferent">EN VEZ DE TRATAR DE SER DIFERENTE,</span><br> SE SIGNIFICATIVO.'
		},
		'au18':{
			'en':'<span class="bold">The common brand</span> are always trying to find new ways to standout from the competitors instead of <span class="bold">looking for the essence inside them</span>, the thing that makes every Brand unique from the rest.',
			'es':'Las <span class="bold">marcas comunes</span> siempre están tratando de encontrar nuevas formas de destacar entre la multitud en vez de  <span class="bold">buscar dentro de ellos la esencia</span>, lo que hace hace a cada marca única del resto.'
		},
		'au19':{
			'en':'To find what makes a Brand unique and truly different, we need to answer these questions before, during and after the creation of the Brand. ',
			'es':'Para encontrar que hace verdaderamete única y diferente a una marca, debemos de responder estas preguntas antes, durante y después del proceso de creación de marca.'
		},
		'au20':{
			'en':'<span class="bold">1. ¿Who are we?</span> We should perfectly know the essence of the Brand, the products and services.',
			'es':'<span class="bold">1. ¿Quién somos?</span> Debemos de conocer a la perfección la esen-cia de la marca, productos y servicios.'
		},
		'au21':{
			'en':'<span class="bold">2. ¿Where are we?</span>  We should acknowledge the objectives of our clients and the competition.',
			'es':'<span class="bold">2. ¿Dónde estamos?</span> Reconocer los objetivos de audiencia y la competencia.'
		},
		'au22':{
			'en':'<span class="bold">3. ¿What do we do?</span>  We need to understand the real needs and problems that the Brand will be able to resolve.',
			'es':'<span class="bold">3. ¿Qué hacemos?</span> Entender las necesidades reales y los problemas que la marca puede resolver.'
		},
		'au23':{
			'en':'<span class="bold">4. ¿How do we do it?</span> Knowing what things of the user experience of the Brand is different from the rest.',
			'es':'<span class="bold">4. ¿Cómo lo hacemos?</span> sabiendo lo que hace que la experiencia del usuario de la marca que es diferente del resto.'
		},
		'au24':{
			'en':'<span class="bold">5. ¿To whom it may concern?</span> We should define goal public (age, sociallanguage, user behavior, media and social media ',
			'es':'<span class="bold">5. ¿A quién va dirigido?</span> Definir público meta (edad, lenguaje social, comportamiento de los consumidores, medios de comuni- cación y redes sociales).'
		},
		'au25':{
			'en':'<span class="bold">6. ¿Why do we do it? </span> We contribute creating the experience of the users.',
			'es':'<span class="bold">6. ¿Por qué lo hacemos?</span> Contribuímos a la experiencia de la usuarios.'
		},
		'au26':{
			'en':'In this way, <span class="bold">your project becomes exceptional in 3 different levels</span>',
			'es':'De esta manera, <span class="bold">su proyecto se convierte en excepcional en 3 niveles diferente</span>'
		},
		'au27':{
			'en':'Even more, this will help us understand towards where are we going and <span class="bold">which will be the right direction to take</span>.',
			'es':'Aún más, esto nos ayudará a entender hacia dónde nos dirigimos, y <span class="bold">cual será la dirección correcta a tomar</span>.'
		},
		'au28':{
			'en':'The projection we have as a result of the Brand Identity is called <span class="bold">Brand image.</span>',
			'es':'La proyección que tenemos como resultado de la identidad se llama <span class="bold">Imagen de la marca</span>.'
		},
		'au29':{
			'en':'The image is represented through a unified Brand system that includes',
			'es':'La imagen es representada a través de un sistema de marca unificado que incluye:'
		},
		'au30':{
			'en':'Branding system',
			'es':'Sistema de marca'
		},
		'au31':{
			'en':'Voice tone',
			'es':'Tono de Voz'
		},
		'au32':{
			'en':'Editorial design',
			'es':'Diseño Editorial'
		},
		'au33':{
			'en':'Logo',
			'es':'Logotipo'
		},
		'au34':{
			'en':'Typography',
			'es':'Tipografía'
		},
		'au35':{
			'en':'Graphic resources',
			'es':'Recursos Gráficos'
		},
		'au36':{
			'en':'Illustration style',
			'es':'Estilo de ilustración'
		},
		'au37':{
			'en':'Color palettes',
			'es':'Paleta de Colores'
		},
		'au38':{
			'en':'High quality pictures',
			'es':'Fotografía de alta calidad'
		},
		'au39':{
			'en':'Iconography',
			'es':'Iconografía'
		},
		'au40':{
			'en':'The Branding that will be evaluated by the public.',
			'es':'Branding que será evaluado por la audiencia.'
		},
		'au41':{
			'en':'Finally it becomes a <span class="bold">Experience design.</span>',
			'es':'Finalmente se convierte en <span class="bold">Diseño de Experiencia</span>'
		},
		'au42':{
			'en':'EXPERIENCE',
			'es':'EXPERIENCIA'
		},
		'au43':{
			'en':'The Road to Victory',
			'es':'El camino a la victoria'
		},
		'au44':{
			'en':'IS PAVED THROUGH STRATEGIES.',
			'es':'ESTÁ PAVIMENTADO A TRAVÉS DE ESTRATEGIAS'
		},
		'au45':{
			'en':'<span class="bold">LeBrand</span>, offers a full range of consultancy and design services, from brand development to specific design work for premium brands: identity, branding, editorial, web design, and visual communication.',
			'es':'<span class="bold">LeBrand</span>, ofrece un alto rango de consultorias y servicios de diseño, desde el desarrollo de la marca hasta un trabajo específico de diseño para las marcas Premium: Identidad, Branding, Diseño Editorial, Diseño Web y comunicación visual.'
		},
		'au46':{
			'en':'<h2 class="title-gray">Identity.</h2><ul><li class="circle-li">Logotype.</li><li class="circle-li">Brand Consulting.</li><li class="circle-li">Identity System.</li><li class="circle-li">Typography.</li><li class="circle-li">Logomark.</li><li class="circle-li">Wordmark.</li><li class="circle-li">Art Direction.</li><li class="circle-li">Creative Direction.</li><li class="circle-li">Naming.</li><li class="circle-li">Slogan.</li><li class="circle-li">Stationery.</li><li class="circle-li">Brand Guidelines.</li></ul>',
			'es':'<h2 class="title-gray">Identidad.</h2><ul><li class="circle-li">Logotipo.</li><li class="circle-li">Consultoría de marca.</li><li class="circle-li">Sistema de identidad.</li><li class="circle-li">Typografía.</li><li class="circle-li">Logotipo.</li><li class="circle-li">Slogan.</li><li class="circle-li">Dirección de arte.</li><li class="circle-li">Dirección creativa.</li><li class="circle-li">Naming.</li><li class="circle-li">Slogan.</li><li class="circle-li">Gráficos estacionarios.</li><li class="circle-li">Lineamientos de la marca.</li></ul>'
		},
		'au47':{
			'en':'<h2 class="title-gray">Branding.</h2><ul><li class="circle-li">Editorial Design.</li><li class="circle-li">Branding System.</li><li class="circle-li">Peripherals.</li><li class="circle-li">Advertising.</li><li class="circle-li">Illustration.</li><li class="circle-li">Iconography.</li><li class="circle-li">Flyers & Invitations.</li><li class="circle-li">Custom Publishing.</li><li class="circle-li">Copywriting.</li></ul>',
			'es':'<h2 class="title-gray">Identidad.</h2><ul><li class="circle-li">Diseño Editorial.</li><li class="circle-li">Sistema de marca.</li><li class="circle-li">Sistema de identidad.</li><li class="circle-li">Periféricos.</li><li class="circle-li">Publicidad.</li><li class="circle-li">Ilustración.</li><li class="circle-li">Iconografía.</li><li class="circle-li">Flyers e Invitaciones.</li><li class="circle-li">Publicaciones personalizadas.</li><li class="circle-li">Copywriting.</li>'
		},
		'au48':{
			'en':'<h2 class="title-gray">Web.</h2><ul><li class="circle-li">User Experience Design <span>(UX)</span>.</li><li class="circle-li">Front-end/HTML+CSS <span>(pixel perfect)</span>.</li><li class="circle-li">Content Managment System.</li><li class="circle-li">Wireframes.</li></ul>',
			'es':'<h2 class="title-gray">Web.</h2><ul><li class="circle-li">Diseño de experiencia <span>(UX)</span>.</li><li class="circle-li">Front-end/HTML+CSS <span>(pixel perfect)</span>.</li><li class="circle-li"> Sistema de gestión de contenidos.	</li><li class="circle-li">Wireframes.</li></ul>'
		},
		'au49':{
			'en':'When the <span class="bold">identity</span> and <span class="bold">image</span> are well established , a powerful and visually consistent <span class="bold">Branding</span> is the result.',
			'es':'Cuando la <span class="bold">identidad</span> y la <span class="bold">imagen</span> estan bien establecidas, un poderoso y visualmente coherente <span class="bold">Branding</span> es el resultado.'
		},
		'au50':{
			'en':'ABOUT',
			'es':'NOSOTROS'
		},
		'au51':{
			'en':'PROCESS',
			'es':'PROCESO'
		},
		'au52':{
			'en':'SERVICES',
			'es':'SERVICIOS'
		},
		'au53':{
			'en':'Work with us to increase the strengths of your brand in the world of today.<br><br> We design meaningful brand experiences, stylishly, seductive so your clients will become your followers.',
			'es':'Trabaja con nosotros para potencializar las fortalezas de tu marca en el mundo de hoy. <br><br> Nosotros diseñamos experiencias de marca significativas, elegantes y seductoras para que sus clientes se convierten en sus seguidores.'
		},
		'au54':{
			'en':'Image',
			'es':'Imagen'
		}


	}
}

var cookie_lang = {};
cookie_lang.setCookie = function(lang){
	document.cookie = lang+';';
}
cookie_lang.getCookie = function(){
	var data = document.cookie.split(';');
	return data[0];
}

function translate_page(page, lang){
	// Indicar idioma en que se va a realizar la navegación por la página
	cookie_lang.setCookie(lang);

	// Cambiar menú de idioma
	for(var key in transale['menu']){
		$('.'+key).html(transale['menu'][key][lang])
	}

	// Cambiar página de idioma
	if(transale[page]){
		for(var key in transale[page]){
			$('.'+key).html(transale[page][key][lang])
		}
	}

	// Cambiar footer de idioma
	for(var key in transale['footer']){
		$('.'+key).html(transale['footer'][key][lang])
	}

	// Cambiar aboutUs de idioma
	for(var key in transale['aboutUs']){
		$('.'+key).html(transale['aboutUs'][key][lang])
	}

	for(var key in transale['index']){
		$('.'+key).html(transale['index'][key][lang])
	}



}

// Al cargar la página poner los textos en el idioma que ya se ha seleccionado
function init_lang(page){
	// Los textos están predefinidos en inglés
	if(cookie_lang.getCookie() == 'es'){
    	translate_page(page, 'es');
    }
}

$(document).ready(function(){
	$('#eng_len').addClass('len_selected');
	$('#eng_len').click(function(){
		if($(this).hasClass('len_selected')){
			return;
		}
		$(this).addClass('len_selected');
		//$(this).css('color','#888885');
		$('#es_len').removeClass('len_selected');
		//$('#es_len').css('color','white');
	});

	$('#es_len').click(function(){
		if($(this).hasClass('len_selected')){
			return;
		}
		$(this).addClass('len_selected');
		//$(this).css('color','#888885');
		$('#eng_len').removeClass('len_selected');
		$//('#eng_len').css('color','white');
	});

});

